    ...
    public class MarkdownHighlightingColorizer : DocumentColorizingTransformer
    {
        ...
        protected override void ColorizeLine(DocumentLine line)
        {
            ...
            foreach (var block in AbstractSyntaxTree.EnumerateSpanningBlocks(ast, start, end))
            {
                ...
                foreach (var inline in AbstractSyntaxTree.EnumerateInlines(block.InlineContent)
                    .TakeWhile(il => il.SourcePosition < end)
                    .Where(il => AbstractSyntaxTree.InlineHighlighter.TryGetValue(il.Tag, out highlighter)))
                {
        ...
        private void ApplyLinePart(IHighlight highlight, int sourceStart, int sourceLength, int lineStart, int lineEnd,
        ...
        private static void ApplyHighlight(VisualLineElement element, IHighlight highlight, double magnify)
        {
